...
class _ResetLessonsPageState extends State<ResetLessonsPage>
 {
  ...
  Widget build(BuildContext context) {
    lessons ??= [];
    return Scaffold(
      appBar: AppBar(backgroundColor: Colors.red, actions: [IconButton(icon: const Icon(Icons.home_outlined,), onPressed: () {kIsWeb? launchUrl(Uri.parse("https://app.shcmusiclessonrolls.com/",), webOnlyWindowName: "_self"):print("auto restart doesn't work here");},)], title: const Text("Reset all your lessons"),),
      body: ListView(
        children: [
          const Text("To cancel, press the home button in the top right. \n\nAll of your lessons will be deleted. Please check that this is what you are expecting to see:", style: TextStyle(fontSize: 30),),
      ...
      floatingActionButton: FloatingActionButton.extended(onPressed: () async {
        showDialog(barrierDismissible: false,context: context, builder: (context) => const AlertDialog(title: Text("Resetting..."),));
        for (var lesson in lessons!) {
          try {
      ...
      }, label: const Text("Reset (this action can't be easily reversed!!)"), backgroundColor: Colors.red,),
    );
  }
}