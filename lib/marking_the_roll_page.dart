...
class _MarkingRollPageState extends State<MarkingRollPage> {
  ...
  void initState() {
    super.initState();
    isCheckedList =
        List.filled(widget.lessonDetails['expand']['students'].length, false);
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (int.parse(widget.lessonDetails['time'].substring(0, 2)) > 12) {
      i12hrTime =
          "${int.parse(widget.lessonDetails['time'].substring(0, 2)) - 12}:${widget.lessonDetails['time'].substring(2, 4)} PM";
    } else {
      i12hrTime =
          "${widget.lessonDetails['time'].substring(0, 2)}:${widget.lessonDetails['time'].substring(2, 4)} AM";
    }
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              Spacer(),
              Text("${widget.lessonDetails['instrument']}",
                  style: TextStyle(fontSize: 30)),
            ],
          ),
        ),
        for (int x = 0;
            x < widget.lessonDetails['expand']['students'].length;
            x++) ...[
          Padding(
              ...
              child: Row(
                children: [
                  Padding(
                    ...
                    child: Text(
                        widget.lessonDetails['expand']['students'][x]['name']),
                  ),
                  Spacer(),
                  Checkbox(
                      value: isCheckedList[x],
                      onChanged: (value) => setState(() {
                            isCheckedList[x] = value!;
                            if (value == true) {
                              presentStudents.add(widget.lessonDetails['expand']
                                  ['students'][x]);
                            } else {
                              presentStudents.remove(widget
                                  .lessonDetails['expand']['students'][x]);
                            }
                ...
                Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) => ConfirmPage(
                              ...
                              lessonDetails: widget.lessonDetails,
                            )));
              },
              child: Text("Submit")),
        )
...
class ConfirmPage extends StatelessWidget {
  ...
  final lessonDetails;
  const ConfirmPage(
      {super.key, required this.presentStudents, required this.lessonDetails});
